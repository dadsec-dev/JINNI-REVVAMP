...
const Home = () => {
  ...
  return (
    <div className="bg-[#319a46]">
      <div className="flex  max-w-6xl mx-auto flex-col items-center justify-center py-2 min-h-screen">
        <Head>
          <title>JINNI AI Luxury Revamped</title>
        </Head>
  ...
  <h1 className="mx-auto max-w-4xl font-display text-5xl font-bold tracking-normal text-[#f3f3f3] sm:text-7xl">
    AI-Powered Excellence in{" "}
    <span className="relative whitespace-nowrap text-[#f3f3f3]">
      <SquigglyLines />
  ...
  <Image alt="Jinni hero Image" src="/jinnihero.png" width={400} height={400} />
</div>
          ...
          <h1 className="mt-8 text-3xl ">
            {Math.floor(data?.formatted)}{" "}
            <span className="text-[#000000] font-bold">{data?.symbol}</span>
          </h1>
          ...
          <div className="hover:bg-[#000000]">
            {data?.value !== undefined &&
              (data.value / BigInt(10 ** 18) >= BigInt(10000000) ? (
                ...
                <button
                  disabled
                  className="bg-gray-600 rounded-xl text-white font-medium px-4 py-3 sm:mt-10 mt-8 hover:bg-greeb-500 transition"
                >
                  Enter Studio
                </button>
              ))}
